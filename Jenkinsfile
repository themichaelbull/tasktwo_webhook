    ...
    stages {
        ...
        stage('run'){
            steps {
                ...
                sh "cd tasktwo_webhook/nginx && pwd && docker run -d -p 80:80 --name nginxapp --network new-network --mount type=bind,source=$(pwd)/nginx.conf,target=/etc/nginx/nginx.conf nginxapp"
            }
        }
    }
}
