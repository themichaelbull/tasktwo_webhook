    ...
    stages {
        ...
        stage('run'){
            steps {
                ...
                sh "cd tasktwo_webhook/nginx && pwd && docker run -dt -p 80:80 --name nginxapp --network new-network --mount type=bind,source=/var/lib/jenkins/workspace/tasktwo_webhook/tasktwo_webhook/nginx/nginx.conf,target=/etc/nginx/nginx.conf nginxapp"
                sh "docker network connect new-network nginxapp"
            }
        }
    }
}
